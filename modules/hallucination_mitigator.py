...
class HallucinationMitigator :
    ...
    def mitigate(
        ...
        for data_id in data_ids:
            ...
            if len(hd_result) == 0:
                results.append(HmResult(data_id, raw_response, raw_response[0]['text']))
                continue
    ...
    def get_instructions_by_hd_results(hd_results = List[HdResult]) -> str:
        ...
        final_instructions = ""
        for i, hd_result in enumerate(hd_results_dedup):
            final_instructions += \
                f"Rewrite instruction {i+1}:\n" + \
                f"Rrwrite sentence in raw_response: {hd_result.hallucinated_sentence}\n" + \
                f"Reason for rewrite the sentence: {hd_result.instruction}\n"
    ...
    def create_payload(item, promptUtil : hallucination_mitigation_prompt) -> Dict:
        ...
        prompt_to_send_to_gpt = promptUtil.create_prompt(
            source = item['source'], 
            raw_response = item['raw_response'][0]['text'],
