...
def create_openai_arguments(config_setting_key : str, max_parallelism : int, config_file : str = None ) -> OpenaiArguments:
    ...
    openai_args.use_chat_completions = settings['USE_CHAT_COMPLETIONS']
    openai_args.max_parallelism = max_parallelism
    openai_args.agents = settings['agents']
    openai_args.rounds = settings['rounds']
    return openai_args
...
class DetectionArguments:
    ...
    # 原项目：tmp = 0.6, top_p = 0.6, max_tokens = 2048, freq_penalty = 0, presence_penalty = 0, log_prob = 0, batch_size = 10, generations = 1

    temp: Optional[float] = field(
        default=1.0, metadata={"help": "What sampling temperature to use, between 0 and 2. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic."}
    )
    top_p: Optional[float] = field(
        default=1, metadata={"help": "An alternative to sampling with temperature, called nucleus sampling, where the model considers the results of the tokens with top_p probability mass. So 0.1 means only the tokens comprising the top 10% probability mass are considered."}
    )
