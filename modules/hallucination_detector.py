...
class HallucinationDetector :
    ...
    @staticmethod
    def construct_message(agents, idx, original_prompt):
        if len(agents) == 0:
            # 已调整prompt
            return {"role": "user", "content": "Please carefully verify that your hypothesis is faithful to the original text.  Reiterate your analysis and ensure that there are no ungrounded hallucinations in your final assessment."}
        # 已调整prompt
        prefix_string = "Here are the analyses of the hypothesis provided by other agents: "

        for agent in agents:
            agent_response = agent[idx]["content"]
            response = "\n\nOne agent's response: ```{}```".format(agent_response)  #已调整prompt
            prefix_string += response

        # 已调整prompt
        prefix_string += """\n\nUsing the responses from other agents as additional information, can you provide your answer? The original prompt is: {}""".format(
            original_prompt[-1]['content'])
    ...
    def multi_agent_gpt_response(self, prompt, aoaiUtil: AOAIUtil, agents, rounds, detection_args: DetectionArguments):
        agent_contexts = [prompt[:] for _ in range(agents)]

        def process_agent(agent_index, agent_context, round):
            if round != 0:
                # 排除当前智能体，构建其他智能体的上下文
                agent_contexts_other = agent_contexts[:agent_index] + agent_contexts[agent_index + 1:]
                message = self.construct_message(agent_contexts_other, 2 * round + 4, prompt)
            ...
            assistant_message = self.construct_assistant_message(gpt_response.choices[0].message.content)
            agent_context.append(assistant_message)

        # 外部循环控制每一轮
        max_threads = min(agents, 4)  # 设置最大线程数
        for round in range(rounds):
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
                # 为每个智能体分配一个线程，传入当前的轮次
                futures = [executor.submit(process_agent, i, agent_contexts[i], round) for i in range(agents)]

                # 等待所有线程完成
                concurrent.futures.wait(futures)

        # 将最后一轮的所有智能体的答案整合起来
        outputs = [gpt_output_utils.clean_for_tsv(agent_context[-1]["content"]) for agent_context in agent_contexts]

        return outputs

    # send payload to GPT endpoint and get back the results
    def process_payload_by_GPT(self, payload, aoaiUtil : AOAIUtil, openai_args : OpenaiArguments, detection_args : DetectionArguments) -> Dict:

        outputs = []
        try:
            logging.info(f"Start to call GPT to process {len(payload['items'])} items")
            if openai_args.use_chat_completions == "single_agent":
                gpt_response = aoaiUtil.get_chat_completion(
            ...
            elif openai_args.use_chat_completions == "multi_agent":
                #已修改todo: openai_args.agents is not defined, agents is not defined
                payload['gpt_raw_output'] = self.multi_agent_gpt_response(payload['prompt'], aoaiUtil, openai_args.agents, openai_args.rounds, detection_args)
            else:
    ...
    def parse_gpt_results(gpt_results_raw) -> List[Dict]:
        ...
        for gpt_result_raw in gpt_results_raw:
            hallucination_results = HallucinationDetector.parse_gpt_results_single(gpt_result_raw)
            if len(hallucination_results) >= math.ceil(len(gpt_result_raw['gpt_raw_output'])/2):
                # todo:优化reason
                gpt_results_cooked.append(hallucination_results[0])
