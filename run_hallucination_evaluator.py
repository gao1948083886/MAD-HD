...
def hallucination_evaluation(jsonl: str):
    ...
    for line in open(jsonl):
        ...
        hallucination_count.append(len(hallucinations))
    return {
        "non-hallucinated response ratio:": sum(0 if num !=0 else 1 for num in hallucination_count)/data_num,
        "max hallucination count per response": max(hallucination_count),
        "min hallucination count per response": min([num for num in hallucination_count if num != 0]),
        }
